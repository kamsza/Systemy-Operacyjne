...
struct message* to_message(char *command) {
	char* command_cpy = malloc(MESSAGE_SIZE);
	sprintf(command_cpy, "%s\n", command);

	char *client_id = strtok(command_cpy, " \n");
	char *cmd = strtok(NULL, " \n");
	char *args = strtok(NULL, "\n");
	
	struct message* msg = malloc(sizeof(struct message));
	msg->client_id = atoi(client_id);
	...
	else if( strcmp(cmd, "INIT") == 0 )		msg->request_id = INIT;
	...
	else	{ printf("MESSAGE to_message: command: %s not found\n", cmd); msg->request_id = ERROR; }
...
char* get_command(struct message* msg) {
	...
	switch (msg->request_id) {
		...
		case ADD: 		return "ADD";
        case DEL: 		return "DEL";			
        default:		return "MESSAGE get_command: message not found\n";
    }
    
}

char* to_string(struct message* msg) {
	char* command = malloc(MESSAGE_SIZE);
	sprintf(command, "%ld %s %s", msg->client_id, get_command(msg), msg->mtext);
	return command;
}
